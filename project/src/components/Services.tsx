...
import { Clock, DollarSign, Sparkles, Share2, Check } from 'lucide-react';
...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, isSelected, onSelect }) => {
  ...
  const handleShare = (e: React.MouseEvent) => {
    e.stopPropagation();
    const shareUrl = `${window.location.origin}?service=${service.id}&name=${encodeURIComponent(service.name)}`;
  ...
  return (
    ...
    >
      {service.promotion_active && service.promotion_price && (
        <div className="absolute -top-2 md:-top-3 -right-2 md:-right-3 bg-gradient-to-r from-red-500 via-red-600 to-red-700 text-white px-3 py-1 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-bold shadow-lg animate-pulse z-20">
          PROMOÇÃO
        </div>
      )}
      {service.popular && !service.promotion_active && (
      ...
      <div className="p-4 md:p-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-2 gap-2">
            <h4 className="text-lg md:text-xl font-semibold text-gray-900 group-hover:text-clinic-600 transition-colors duration-300 flex-1">{service.name}</h4>
            <button
              onClick={handleShare}
              className={`flex-shrink-0 p-1 md:p-2 rounded-lg transition-all duration-300 ${
                isCopied
                  ? 'bg-green-100 text-green-600'
                  : 'bg-gray-100 text-gray-600 hover:bg-clinic-100 hover:text-clinic-600'
              }`}
          ...
          <div className="flex justify-between items-center mb-3 md:mb-4">
            <div className="flex flex-col space-y-1">
              {service.promotion_active && service.promotion_price ? (
                <>
                  <div className="flex items-center space-x-1 text-gray-400 line-through text-xs">
                    <span>De: R$ {service.price}</span>
                  </div>
                  <div className="flex items-center space-x-1 text-red-600 group-hover:text-red-700 transition-colors duration-300">
                    ...
                    <span className="font-bold text-lg md:text-xl">R$ {service.promotion_price}</span>
                  </div>
                </>
              ) : (
...
const Services = ({ services, onServiceSelect, selectedServices }) => {
  ...
  const categories = [...orderedCategories, ...newCategories];
  ...
  return (
    <section className="py-8 md:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Category Filters with Popular */}
        {categories.length > 0 && (
          <div className="flex flex-wrap justify-center gap-2 md:gap-4 mb-8 md:mb-12">
            {popularServices.length > 0 && (
              <button
                onClick={() => setShowPopular(!showPopular)}
                className={`px-4 py-2 md:px-6 md:py-3 rounded-full font-medium text-sm md:text-base transition-all duration-300 transform hover:scale-105 flex items-center gap-2 ${
                  showPopular
            ...
            {categories.map(category => (
              <button
                ...
                className={`px-4 py-2 md:px-6 md:py-3 rounded-full font-medium text-sm md:text-base transition-all duration-300 transform hover:scale-105 ${
                  selectedCategory === category && !showPopular
                    ? 'bg-gradient-to-r from-clinic-500 to-clinic-600 text-white shadow-lg'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {(showPopular ? popularServices : filteredServices).map(service => (
            <ServiceCard
              key={service.id}
              service={service}
        ...
        {(showPopular ? popularServices : filteredServices).length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              {showPopular
                ? 'Não há terapias marcadas como "Mais Procurados".'
