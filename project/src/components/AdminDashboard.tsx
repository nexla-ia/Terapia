...
const AdminDashboard = ({ salon, onLogout }: AdminDashboardProps) => {
  ...
  const [editPromoInput, setEditPromoInput] = useState('');
  ...
  const loadData = async () => {
    try {
      ...
      if (servicesResult.data) {
        const sortedServices = [...servicesResult.data].sort((a, b) => {
  ...
  const handleUpdateService = async (service: Service) => {
    try {
      ...
      if (data) {
        setServices(prev => {
          const updated = prev.map(s => s.id === service.id ? data : s);
          return updated.sort((a, b) => {
            if (a.popular && !b.popular) return -1;
            if (!a.popular && b.popular) return 1;
            return 0;
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="flex items-center">
                        <input
                          type="checkbox"
                          checked={newService.promotion_active}
                          onChange={(e) => setNewService(prev => ({
                            ...prev,
                            promotion_active: e.target.checked,
                            promotion_price: e.target.checked ? prev.promotion_price : 0
                          }))}
                          className="rounded border-gray-300 text-red-600 focus:ring-red-500"
                        />
                        <span className="ml-2 text-sm text-gray-700 font-medium">Promoção ativa</span>
                      </label>
                    </div>
                    {newService.promotion_active && (
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const normalizedValue = value.replace(',', '.');
                            const numValue = normalizedValue === '' ? 0 : parseFloat(normalizedValue) || 0;
                            setNewService(prev => ({ ...prev, promotion_price: numValue }));
                          }}
                          onFocus={(e) => e.target.select()}
                          onBlur={(e) => {
                            if (e.target.value === '') {
                              setPromoInput('');
                              setNewService(prev => ({ ...prev, promotion_price: 0 }));
                            }
                          }}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="flex items-center">
                            <input
                              ...
                              onChange={(e) => setEditingService(prev => prev ? ({ ...prev, popular: e.target.checked }) : null)}
                              className="rounded border-gray-300 text-clinic-600 focus:ring-clinic-500"
                            />
                            <span className="ml-2 text-sm text-gray-700">Serviço popular</span>
                          </label>
                        ...
                        <div>
                          <label className="flex items-center">
                            <input
                              type="checkbox"
                              checked={editingService.promotion_active}
                              onChange={(e) => setEditingService(prev => prev ? ({
                                ...prev,
                                promotion_active: e.target.checked,
                                promotion_price: e.target.checked ? prev.promotion_price : null
                              }) : null)}
                              className="rounded border-gray-300 text-red-600 focus:ring-red-500"
                            />
                            <span className="ml-2 text-sm text-gray-700 font-medium">Promoção ativa</span>
                        ...
                        {editingService.promotion_active && (
                          <div>
                            <label className="block text-sm font-medium text-red-700 mb-1">Preço Promocional (R$)</label>
                            <input
                              type="text"
                              ...
                              onChange={(e) => {
                                const value = e.target.value.replace(/[^\d.,]/g, '');
                                setEditPromoInput(value);
                                const normalizedValue = value.replace(',', '.');
                                const numValue = normalizedValue === '' ? 0 : parseFloat(normalizedValue) || 0;
                                setEditingService(prev => prev ? ({ ...prev, promotion_price: numValue }) : null);
                              }}
                              onFocus={(e) => e.target.select()}
                              onBlur={(e) => {
                                if (e.target.value === '') {
                                  setEditPromoInput('');
                                  setEditingService(prev => prev ? ({ ...prev, promotion_price: 0 }) : null);
                                }
                              }}
                              className="w-full px-3 py-2 border border-red-300 rounded-md focus:ring-2 focus:ring-red-500 focus:border-transparent"
                              placeholder="0,00 ou 0.00"
                          ...
                          <div className="flex items-center space-x-4 mt-2 text-sm text-gray-600">
                            {service.promotion_active && service.promotion_price ? (
                              <span className="flex items-center gap-2">
                                <span className="line-through text-gray-400">R$ {service.price}</span>
                                <span className="text-red-600 font-bold">R$ {service.promotion_price}</span>
                              </span>
                            ) : (
                            ...
                            {service.promotion_active && (
                              <span className="bg-red-100 text-red-800 px-2 py-1 rounded text-xs font-bold">PROMOÇÃO</span>
                            )}
                          </div>
                        </div>
                        <div className="flex space-x-2">
                          <button
                            onClick={() => {
                              ...
                              setEditPriceInput(service.price.toString());
                              setEditPromoInput(service.promotion_price ? service.promotion_price.toString() : '');
                            }}
